...
process_wait (tid_t child_tid UNUSED) {
  ...
  list_remove(&child_process_ptr->elem);
  free(child_process_ptr);
...
/* Create a minimal stack by mapping a zeroed page at the top of
   user virtual memory. */
static bool
setup_stack (void **esp, char *init_name) 
{
  uint8_t *kpage;
  bool success = false;
  ...
  //Separating Arguments
   for (token = strtok_r (init_name, " ", &save_ptr); token != NULL;
        token = strtok_r (NULL, " ", &save_ptr)) {

        if (argc > len) {
          arr = realloc(arr, 2 * sizeof(char *) * len);
          len *= 2;
        }
  ...
  *esp -= 4;
  ...
  //Adresss to Arguments: Last to First
  int j = argc;
  while( --j >= 0){
    *esp = *esp - sizeof(char*);
    *(int*)*esp = arr[j];
